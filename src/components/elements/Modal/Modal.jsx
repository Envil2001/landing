...
const Modal = ({ mobileMenuActive, setMobileMenuActive }) => {
    const headerMenu = [
        {
            name: "Our team",
            link: "/team",
    ...
    return (
        <>
            {
                mobileMenuActive ?
                    <div className="fixed top-0 left-0 w-full h-full z-40 bg-opacity-50 bg-black rounded-t-lg">
                        <div className=" w-full fixed bottom-0 left-0 rounded-t-lg bg-zinc-900">
                            {
                                headerMenu.map((item, index) => (
                                    <Link to={item.link}>
                                        <div key={index} className="Body-13 px-10 flex items-center text-white cursor-pointer
